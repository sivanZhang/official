    ...
    def post(self, request):
        ...
        if  'pic' in request.FILES: # 说明是在上传图库图片 
            ...
            for file_item in files:
                ...
                pic = DetailPic.objects.create(url=filename.replace('\\', '/')) 
            result['status'] = 'OK'
            result['msg']    = _('Uploaded...')
            content_text = """
            <script type=\"text/javascript\"> 
            window.parent.CKEDITOR.tools.callFunction(92, '{0}', '' ); 
            </script>
            """.format(pic.url)
            print(pic.url)
            return HttpResponse(content_text, content_type="text/html")
